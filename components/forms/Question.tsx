					...
					render={({ field }) => (
						<FormItem className="flex w-full flex-col">
							...
							<FormControl className="mt-3.5">
								{/* Might change the background to light 900 well see */}
								<Input className="no-focus paragraph-regular background-light900_dark300 light-border-2 text-dark300_light700 min-h-[20px] border" {...field} />
							</FormControl>
									...
									{field.value.length > 0 && (
										<div className="flex-start mt-2.5 gap-2.5">
											{field.value.map((tag: any) => (
												<Badge key={tag} className="subtle-medium background-light700_dark300 text-light400_light500 flex items-center justify-center gap-2 rounded-md border-none px-4 py-2 capitalize" 
												>
													{tag}
													<Image 
